...
여러명의 상담사가 있을 경우 고객대응을 효과적으로 분배


![WS Setting Routings](assets/images/ws-settings-routing/wsSettingRoutings.png)

### 일반

#### 자동 배정

고객이 상담사 연결을 할 경우, 자동으로 상담사에게 배정됩니다.
* 기본 방법(실시간 상담수가 적을수록)
  - 배정된 상담수가 적은 상담사를 우선
  - 배정된 상담수가 같다면 상담을 마지막으로 종료한 시간이 오래된 상담사를 우선
* 최대 동시 상담수
  - 상담사가 최대로 대응할 수 있는 고객수를 조정 가능

    ?> 상담사 자신이 명시적으로 상담을 더 많이 당겨올 수 있음. 예를 들어, 상담사가 최대 동시 상담수에 도달한 상태에서도, 대기 목록의 대기 항목을 "**상담으로 변경**"시 해당 상담사의 상담 리스트에 추가됨.

#### 수동 배정

고객이 상담사 연결을 할 경우, 상담사가 수동으로 선택배정 전까지 대기하게 됩니다.

#### 대화 설정
* **고객** 대화 유지 시간
  - 상담고객이 지정된 시간동안 입력이 없을 경우, 명시적으로 상담을 자동 종료처리 합니다.
* **상담사** 대화 유지 시간
  - 고객 배정이후 상담사가 지정된 시간동안 입력이 없을 경우, 다른 상담사한테 연결을 시도합니다.
    - 이때, 고객을 응대하지 않은 상담사는 자동으로 `바쁨` 상태로 변경됩니다.
    - 단, 수동배정의 경우에는 다른 상담사연결이 아닌 대기열로 이동됩니다.
* 욕설 가리기(시스템적으로 자동 처리)
  - 고객의 욕설을 상담사가 보지 않을 수 있도록 *** 와 같이 표시 됩니다.
